...
function buildBlueskyText(): string {
  return [
    ...
    '',
    'サブスク済み ChatGPT / Claude / Gemini に貼るだけで、論文専用アシスタントに変身。回数無制限。',
    '',
    '【機能】',
    '📖 要点整理 / 📊 統計チェック / ⚠️ 限界 / 🎓 卒論テーマ / ✍️ 引用',
    '',
    TOOLKIT_URL,
  ].join('\n');
}
  ...
  // === X (Twitter) ハイブリッド投稿: API 試行 → 失敗時 メール fallback ===
  {
    ...
    if (args.dryRun) {
      ...
      try {
        ...
        await notifyXPost({
          subject: '[X 投稿 fallback] PubMed Lab toolkit 告知',
          xText,
          contextNote: `TOOLKIT announcement cron 発火時に X 自動投稿を試みましたが失敗しました。手動で投稿してください。
エラー: ${msg}`,
